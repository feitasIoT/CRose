...
class MailThread(models.AbstractModel):
    ...
    def _check_ai_reply(self, message):
        ...
        should_reply = False
        if self._name == "discuss.channel" and deepseek_partner.id in self.channel_partner_ids.ids:
            should_reply = True
        ...
        def trigger_ai_thread():
            _logger.info("DeepSeek: Triggering thread for model %s record %s", model_name, record_id)
    ...
    def _chat_with_ai_worker(self, user_content, model_name, record_id, deepseek_partner_id):
        _logger.info("DeepSeek: Thread started for %s %s", model_name, record_id)
        with self.pool.cursor() as new_cr:
            ...
            try:
                ...
                if response.status_code != 200:
                    ...
                    _logger.error("DeepSeek: %s", error_msg)
                    record.with_context(skip_ai_reply=True).message_post(
                        body=error_msg,
                        author_id=deepseek_partner.id,
                        message_type="comment",
                ...
                _logger.info("DeepSeek: Posting reply (len=%s)", len(full_content))
                record.with_context(skip_ai_reply=True).message_post(
                    body=full_content,
                    author_id=deepseek_partner.id,
                    message_type="comment",
                ...
                _logger.error("DeepSeek: Exception during chat: %s", error)
                record.with_context(skip_ai_reply=True).message_post(
                    body=f"AI Error: {str(error)}",
                    author_id=deepseek_partner.id,
                )
                self.env.cr.commit()
            finally:
